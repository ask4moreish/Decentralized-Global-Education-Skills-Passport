...
// Execute: requires MAINNET_CONFIRM=SKILLS_PASSPORT_MAINNET and explicit --execute.
...
function printChecklist(bid: bigint, escrow: bigint, execute: boolean) {
  console.log("Decentralized Global Education & Skills Passport — mainnet micro runner\n");
  ...
  if (execute) {
    console.log("  [ ] MAINNET_CONFIRM=SKILLS_PASSPORT_MAINNET is set");
...
async function main() {
  ...
  if (!execute) {
    ...
    console.log("  MAINNET_CONFIRM=SKILLS_PASSPORT_MAINNET OPERATOR_SECRET=S… BIDDER_SECRET=S… \\");
    console.log("    pnpm mainnet:micro -- --execute");
    return;
  }

  if (process.env.MAINNET_CONFIRM !== "SKILLS_PASSPORT_MAINNET") {
    throw new Error('set MAINNET_CONFIRM=SKILLS_PASSPORT_MAINNET to execute on mainnet');
  }
  assertMainnetConfirmed();
  ...
  const operator = new SkillsPassportClient({
    rpcUrl,
    networkPassphrase: network,
    contractId,
  ...
  const bidder = new SkillsPassportClient({
    rpcUrl,
    networkPassphrase: network,
    contractId,

...
async function main() {
  ...
  const roundId = await operator.createRound({
    itemRef: sha256("skills-passport://keeper-e2e/item"),
    revealRound,
    commitDeadline,
    revealDeadline,
  ...
  const bidder = new SkillsPassportClient({ rpcUrl: RPC_URL, networkPassphrase: NETWORK, contractId, secretKey: bidderSecret });
  ...
  const reader = new SkillsPassportClient({ rpcUrl: RPC_URL, networkPassphrase: NETWORK, contractId, publicKey: keeperKp.publicKey() });

...
if [[ "${MAINNET_CONFIRM:-}" != "SKILLS_PASSPORT_MAINNET" ]]; then
  echo "error: set MAINNET_CONFIRM=SKILLS_PASSPORT_MAINNET before mainnet settle" >&2
  exit 1
fi
...
pnpm --filter @decentralized-global-education-skills-passport/keeper exec tsx scripts/mainnet-settle.ts

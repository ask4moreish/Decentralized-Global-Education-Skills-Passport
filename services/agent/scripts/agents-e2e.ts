...
import { generateAuditorKeypair, quicknet } from "skills-passport-tlock";
...
async function main() {
  ...
  const itemRefStr = "skills-passport://agents/spectrum-block-9";
  ...
  try {
    ...
    const keeperSdk = new SkillsPassportClient({
      rpcUrl: RPC_URL,
      networkPassphrase: NETWORK,
      contractId,
    ...
    if (process.env.SKILLS_PASSPORT_WRITE_WEB_TRACE !== "0") {
      await writeDemoTrace(
        process.env.SKILLS_PASSPORT_WEB_DEMO_TRACE_OUT ?? "apps/web/src/demo/demo-trace.generated.ts",
        demoTrace,
      );
    }

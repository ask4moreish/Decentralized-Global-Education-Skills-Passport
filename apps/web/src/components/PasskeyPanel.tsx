...
const LINKS = [
  ...
  },
  {
    ...
    note: "Optional fee sponsor for passkey-signed txs; Decentralized Global Education & Skills Passport uses direct Soroban RPC today.",
  },
  {
...
export function PasskeyPanel() {
  ...
  async function createDemoKey() {
    ...
    try {
      ...
      const created = await account.createKey("Decentralized Global Education & Skills Passport", passkeyUserId(), {
        rpId,
      });
      setWallet({
  ...
  async function deploySmartWallet() {
    ...
    try {
      const { PasskeyKit } = await import("passkey-kit");
      const account = new PasskeyKit(passkeyKitOptions());
      const created = await account.createWallet("Decentralized Global Education & Skills Passport", passkeyUserId(), {
        rpId,
      });
      setWallet({
        keyId: created.keyIdBase64 ?? created.keyId,
